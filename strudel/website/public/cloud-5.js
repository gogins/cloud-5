...
class Cloud5Piece extends HTMLElement {
  constructor() {
    ...
    // Normalize a URL specifier to something the browser can resolve.
    // Accepts http(s), data:, blob:, absolute (/...), relative (./ or ../), or bare (no slash).
    // Bare specifiers are mapped to the web root ("/" + spec).
    _normalizeUrl(u) {
      if (!u) return u;
  ...
  */
  ...
  //await audioContext.audioWorklet.addModule(new URL('./CsoundAudioProcessor.js', import.meta.url));
 ...
 */
// Cloud5Repl — minimal wrapper around @strudel/embed
// Usage:
//   <cloud5-repl embed-src="./vendor/strudel/embed.js">
//     <!-- your Strudel code here -->
//   </cloud5-repl>
//
// JS:
//   const r = document.querySelector('cloud5-repl');
//   r.setCsound(window.csound);      // optional: parent-owned csound
//   r.setCsoundAC(window.CsoundAC);  // optional
//   r.start(); // starts Csound (if available) + Strudel
//   r.stop();

/**
 * <cloud5-strudel>
 * No-iframe Strudel REPL wrapper running in the page's JS realm.
 * Integrates with Csound via AudioWorklet (CsoundAudioNode + CsoundAudioProcessor).
 *
 * Assumes the following live at the Web root (/strudel/website/public → served as "/"):
 *   /CsoundAC.js              (classic script, optional – exposes window.CsoundAC)
 *   /CsoundAudioNode.js       (ES module – wrapper around AudioWorkletNode)
 *   /CsoundAudioProcessor.js  (worklet processor module, registered via addModule)
 *   /csoundac.mjs             (ES module alternative to CsoundAC.js)
 *
 * Attributes (all optional):
 *   data-csound-node   - URL of CsoundAudioNode.js (default: "/CsoundAudioNode.js")
 *   data-csound-proc   - URL of CsoundAudioProcessor.js (default: "/CsoundAudioProcessor.js")
 *   data-csoundac      - URL of csoundac.mjs (default: "/csoundac.mjs")
 *   use-window         - reuse window.cloud5AudioCtx / window.cloud5CsoundNode if present
 *   code               - initial Strudel program (otherwise reads inner HTML comment)
 *   autostart          - if present, calls play() on connect (subject to autoplay policy)
 */
/**
 * <cloud5-strudel>
 * No-iframe Strudel REPL wrapper running in the page's JS realm.
 ...
 *
 * Assumes the following live at the Web root (/strudel/website/public → served as "/"):
 *   /CsoundAC.js              (classic script – defines window.CsoundAC)
 *   /CsoundAudioNode.js       (ES module – wrapper around AudioWorkletNode)
 *   /CsoundAudioProcessor.js  (worklet processor module, registered via addModule)
 *   /csoundac.mjs             (ES module – Strudel helpers/patterns for CsoundAC; MUST be loaded)
 *
 * Attributes (all optional):
 *   data-csoundac-classic - URL of classic CsoundAC.js (default: "/CsoundAC.js")
 *   data-csound-node      - URL of CsoundAudioNode.js (default: "/CsoundAudioNode.js")
 *   data-csound-proc      - URL of CsoundAudioProcessor.js (default: "/CsoundAudioProcessor.js")
 *   data-csoundac         - URL of csoundac.mjs (default: "/csoundac.mjs")
 *   use-window            - reuse window.cloud5AudioCtx / window.cloud5CsoundNode if present
...
class Cloud5Strudel extends HTMLElement {
  static get observedAttributes() { return ['code', 'autostart']; }
  ...
  constructor() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host { display:block; }
        strudel-editor { display:block; min-height:320px; }
      </style>
      <strudel-editor id="repl"></strudel-editor>
    `;
    this.repl = this.shadowRoot.getElementById('repl');
  }
  
  // Normalize a URL for dynamic import/addModule:
  // Accepts http(s), data:, blob:, absolute (/...), relative (./ or ../), or bare (no slash).
  // Bare specifiers are mapped to the web root ("/" + spec).
  _normalizeUrl(u) {
    if (!u) return u;
  ...
  async connectedCallback() {
    // Helper to load a classic script tag (for CsoundAC.js)
    ...
    // 1) Ensure a shared AudioContext
    if (!window.cloud5AudioCtx) {
      window.cloud5AudioCtx = new (window.AudioContext || window.webkitAudioContext)({ latencyHint: 'interactive' });
    }
    this.audioCtx = window.cloud5AudioCtx;

    // 2) Ensure CsoundAC global exists (classic), then ALWAYS load csoundac.mjs for Strudel helpers
    const acClassicUrl = this._normalizeUrl(this.getAttribute('data-csoundac-classic') || '/CsoundAC.js');
    const acMjsUrl     = this._normalizeUrl(this.getAttribute('data-csoundac')         || '/csoundac.mjs');

    try {
      if (!window.CsoundAC) {
        await loadClassic(acClassicUrl);
      }
    } catch (e) {
      console.warn('[cloud5-strudel] Could not load classic CsoundAC.js:', acClassicUrl, e);
    }

    try {
      // Load mjs helpers/patterns regardless of classic availability (side-effects expected)
      this.csoundACHelpers = await import(acMjsUrl);
    } catch (e) {
      console.warn('[cloud5-strudel] Could not import csoundac.mjs:', acMjsUrl, e);
    }

    // 3) Set up Csound worklet + node wrapper in THIS realm
    const useWindow = this.hasAttribute('use-window');
    const nodeUrl = this._normalizeUrl(this.getAttribute('data-csound-node') || '/CsoundAudioNode.js');
    const procUrl = this._normalizeUrl(this.getAttribute('data-csound-proc') || '/CsoundAudioProcessor.js');
    const procName = this.getAttribute('processor-name') || 'CsoundAudioProcessor';
    let nodeOptions = {};
    try { const raw = this.getAttribute('node-options'); if (raw) nodeOptions = JSON.parse(raw); } catch (e) { console.warn('[cloud5-strudel] Invalid JSON in node-options attribute:', e); }


    if (useWindow && window.cloud5CsoundNode) {
      this.csoundNode = window.cloud5CsoundNode;
    } else {
      try {
        console.debug('[cloud5-strudel] addModule:', procUrl);
        await this.audioCtx.audioWorklet.addModule(procUrl);
      } catch (e) {
        console.error('[cloud5-strudel] audioWorklet.addModule failed:', procUrl, e);
        throw e;
      }
      
      try {
        console.debug('[cloud5-strudel] dynamic import:', nodeUrl);
        let created = null;
        let nodeMod = null;
        try {
          nodeMod = await import(nodeUrl);
        } catch (impErr) {
          console.warn('[cloud5-strudel] import() failed, will try classic script:', impErr);
        }

        if (nodeMod) {
          const keys = Object.keys(nodeMod || {});
          console.debug('[cloud5-strudel] CsoundAudioNode exports:', keys);
          if (typeof nodeMod.createCsoundAudioNode === 'function') {
            created = await nodeMod.createCsoundAudioNode(this.audioCtx, Object.assign({ processorUrl: procUrl }, nodeOptions));
          } else if (typeof nodeMod.createNode === 'function') {
            created = await nodeMod.createNode(this.audioCtx, Object.assign({ processorUrl: procUrl }, nodeOptions));
          } else if (typeof nodeMod.makeCsoundAudioNode === 'function') {
            created = await nodeMod.makeCsoundAudioNode(this.audioCtx, Object.assign({ processorUrl: procUrl }, nodeOptions));
          } else if (nodeMod.CsoundAudioNode) {
            created = new nodeMod.CsoundAudioNode(this.audioCtx, nodeOptions);
          } else if (typeof nodeMod.default === 'function') {
            try { created = await nodeMod.default(this.audioCtx, Object.assign({ processorUrl: procUrl }, nodeOptions)); }
            catch { created = new nodeMod.default(this.audioCtx, nodeOptions); }
          }
        }

        // If still not created, check globals (in case node file is classic and sets window.*)
        if (!created) {
          if (window.createCsoundAudioNode) {
            console.debug('[cloud5-strudel] using global createCsoundAudioNode(...)');
            created = await window.createCsoundAudioNode(this.audioCtx, Object.assign({ processorUrl: procUrl }, nodeOptions));
          } else if (window.makeCsoundAudioNode) {
            console.debug('[cloud5-strudel] using global makeCsoundAudioNode(...)');
            created = await window.makeCsoundAudioNode(this.audioCtx, Object.assign({ processorUrl: procUrl }, nodeOptions));
          } else if (window.CsoundAudioNode) {
            console.debug('[cloud5-strudel] using global window.CsoundAudioNode constructor');
            created = new window.CsoundAudioNode(this.audioCtx, nodeOptions);
          }
        }

        // If import() failed or gave no exports and no globals existed, try classic script load
        if (!created && !nodeMod) {
          await new Promise((resolve, reject) => {
            const s = document.createElement('script');
            s.src = nodeUrl;
            s.onload = () => resolve();
            s.onerror = () => reject(new Error('Failed to load classic ' + nodeUrl));
            document.head.appendChild(s);
          });
          if (window.createCsoundAudioNode) {
            created = await window.createCsoundAudioNode(this.audioCtx, Object.assign({ processorUrl: procUrl }, nodeOptions));
        ...
        // Last resort: plain AudioWorkletNode
        if (!created) {
          console.warn('[cloud5-strudel] falling back to new AudioWorkletNode(...) with', procName);
          ...
          const opts = Object.assign({}, defaultOptions, nodeOptions);
          if (!opts.processorOptions) opts.processorOptions = {};
          created = new AudioWorkletNode(this.audioCtx, procName, opts);
        }

        this.csoundNode = created;
        if (this.csoundNode.connect && !this._connected) {
          this.csoundNode.connect(this.audioCtx.destination);
          this._connected = true;
        }
        // add basic error logging on the node
        if (this.csoundNode && 'onprocessorerror' in this.csoundNode) {
          this.csoundNode.onprocessorerror = (e) => console.error('[cloud5-strudel] AudioWorklet processor error:', e);
        }
        if (this.csoundNode && this.csoundNode.port) {
          this.csoundNode.port.onmessageerror = (e) => console.error('[cloud5-strudel] worklet port message error:', e);
        }

        window.cloud5CsoundNode = this.csoundNode;
      } catch (e) {

        console.error('[cloud5-strudel] Failed to create CsoundAudioNode:', nodeUrl, e);
      }
    }

    // 4) Seed Strudel code (from attribute or inner comment)
    const comment = (() => {
      for (const n of this.childNodes) if (n.nodeType === Node.COMMENT_NODE) return n.data;
      return '';
    })();
    const initial = this.getAttribute('code') ?? comment ?? '';
    this._setCode(initial);

    // 5) Hook basic play/stop
    this.repl.addEventListener('play', () => this._startAudio());
    this.repl.addEventListener('stop', () => this._maybeSuspend());

    if (this.hasAttribute('autostart')) this.play();
  }

  attributeChangedCallback(name, _old, val) {
    if (name === 'code' && this.isConnected) this._setCode(val || '');
    if (name === 'autostart' && this.isConnected && this.hasAttribute('autostart')) this.play();
  }

  // Public API
  async play() {
    await this._startAudio();
    this.repl.dispatchEvent(new Event('play'));
  }
  stop() {
    this.repl.dispatchEvent(new Event('stop'));
    this._maybeSuspend();
  }
  setCode(code) { this._setCode(code); }

  // Internals
  async _startAudio() {
    if (this.audioCtx && this.audioCtx.state !== 'running') {
      try { await this.audioCtx.resume(); } catch (e) { console.warn('[cloud5-strudel] resume failed:', e); }
    }
    // If your node needs explicit start/init, call it here:
    // await this.csoundNode?.start?.();
  }
  _maybeSuspend() {
    // Optional power saving:
    // this.audioCtx?.suspend?.();
    // or forward to node: this.csoundNode?.stop?.();
  }
  _setCode(code = '') {
    this.repl.innerHTML = `<!--
${code}
-->`;
    this.repl.dispatchEvent(new CustomEvent('codechange', { detail: { code } }));
  }
}
...
/* === Cloud5: Strudel REPL loader (no-iframe) === */
(function (global, d) {
  const Cloud5 = (global.Cloud5 = global.Cloud5 || {});
  Cloud5.plugins = Cloud5.plugins || {};
  if (Cloud5.plugins.strudelLoader) return; // idempotent

  // Config (override if you like)
  const STRUDEL_REPL_URL = "https://unpkg.com/@strudel/repl@latest/dist/index.js";
  const ESMS_URL = "https://unpkg.com/es-module-shims@latest/dist/es-module-shims.js";

  // Feature detect native ESM support from a classic script
  const supportsESM = "noModule" in d.createElement("script");

  function loadScript({ src, type, text }) {
    return new Promise((resolve, reject) => {
      ...
      s.onload = () => resolve();
      s.onerror = () => reject(new Error("Failed to load " + (src || type)));
      d.head.appendChild(s);
    });
  }

  function whenStrudelReady() {
    if (customElements.get("strudel-editor")) return Promise.resolve();
    return customElements.whenDefined ? customElements.whenDefined("strudel-editor") : Promise.resolve();
  }

  // Public API
  Cloud5.ensureStrudelRepl = async function ensureStrudelRepl() {
    if (customElements.get("strudel-editor")) return; // already loaded

    if (supportsESM) {
      // Load the web component via a tiny inline module
      await loadScript({ type: "module", text: `import "${STRUDEL_REPL_URL}";` });
    } else {
      // Old browsers: load es-module-shims, then import via module-shim
      await loadScript({ src: ESMS_URL });
      await loadScript({ type: "module-shim", text: `import "${STRUDEL_REPL_URL}";` });
    }
    await whenStrudelReady();
  };

  Cloud5.getRepl = (sel = "strudel-editor") => d.querySelector(sel);

  Cloud5.setStrudelCode = (el, code) => {
    el = el || Cloud5.getRepl();
    if (!el) return;
    el.innerHTML = `<!--\n${code}\n-->`;
    el.dispatchEvent(new CustomEvent("codechange", { detail: { code } }));
  };

  Cloud5.play = (el) => (el || Cloud5.getRepl())?.dispatchEvent(new Event("play"));
  Cloud5.stop  = (el) => (el || Cloud5.getRepl())?.dispatchEvent(new Event("stop"));

  // Optional auto-init: load REPL if a <strudel-editor> already exists in DOM
  if (d.querySelector("strudel-editor")) {
    Cloud5.ensureStrudelRepl().catch(console.error);
  }

  Cloud5.plugins.strudelLoader = true;
})(window, document);
/* === /Cloud5: Strudel REPL loader === */
