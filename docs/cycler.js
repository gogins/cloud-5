...
///UNCLEAR: cycle time setting for children of Node, Stack, Sequence, and Cycle.
...
class Node {
  ...
  traverse(cycle_time, depth, parent_score) {
    cycler_log("[traverse] cycle_start: %9.4f depth: %5d events: %6d", cycle_time, depth, parent_score.size());
    ...
    for (let child of this.children) {
      ...
      for (let i = 0; i < child.score.size(); ++i) {
...
class Cycle extends Node {
  ...
  render() {
    ...
    cycler_log("[Cycle][render] traversal interval:        %9.4f score duration: %9.4f", traversal_interval, this.score.getDurationFromZero());
    this.csound.readScore(score_text);
    this.current_rendering_time = this.performance_time();
    cycler_log("[Cycle][render] prior_rendering_time:      %9.4f current_rendering_time: %9.4f", this.prior_rendering_time, this.current_rendering_time);
    cycler_log("[Cycle][render] actual rendering interval: %9.4f", this.current_rendering_time - this.prior_rendering_time);
  ...
  cycle() {
    ...
    // Traverse the graph one time, setting cycle time to 0.
