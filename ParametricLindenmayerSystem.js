    ...
    ParametricLindenmayer.Turtle = class {
        constructor(note_, chord_, scale_) {
            ...
            if (typeof scale_ === "undefined") {
                this.scale = new ChordSpace.Chord();
            } else {
    ...
    ParametricLindenmayer.PLSystem = class {
        constructor() {
            ...
            this.add_command('Assign(t, d, s, c, k, v, x)', function (lsystem, turtle, t, d, s, c, k, v, x) {
                turtle.note.time = (t * turtle.magnitude[0]);
                turtle.note.duration = (d * turtle.magnitude[1]);
                turtle.note.status = (s * turtle.magnitude[2]);
                turtle.note.channel = (c * turtle.magnitude[3]);
                turtle.note.key = (k * turtle.magnitude[4]);
                turtle.note.velocity = (v * turtle.magnitude[5]);
                turtle.note.pan = (x * turtle.magnitude[6]);
                return turtle;
            });
